...
function startQuiz(event)
{
	...
	function displayQuestion(index)
	{
		...
		const submitButon = getSubmitButton();

		submitButon.addEventListener("click", submit);
		app.appendChild(submitButon);
	}
...
function getFeedbackMessage(isCorrect, correct)
{
		...
		paragraph.innerText = isCorrect ? 
			...
			: `Désole... mais la bonne reponse etait ${correct}`;
...
function displayNextQuestionButton(callback)
		...
		const getButtonText = () =>
		{
			return `Next (${remainingTimeout / 100}s)`;
		}
